...
one_rupee_coin = cv2.imread('removebg.png', -1)
...
# Initialize one rupee coin position
coin_size_ratio = 0.2  # Size ratio of the one rupee coin
coin_width = int(width * coin_size_ratio)
coin_height = int(coin_width * one_rupee_coin.shape[0] / one_rupee_coin.shape[1])
coin_x = int((width - coin_width) / 2)  # Centered horizontally
coin_y = int(height / 4)  # Positioned vertically at one-fourth height
...
while True:
    ...
    # Apply face mesh overlay, head hat, and one rupee coin to each detected face
    for i in indices:
        i = i  # Extract the index from the tuple
        ...
        if results.multi_face_landmarks:
            ...
            # Resize one rupee coin to match the forehead size
            coin_resized = cv2.resize(one_rupee_coin, (coin_width, coin_height))

            # Overlay one rupee coin on the forehead
            coin_region = frame[forehead_y:forehead_y + coin_height, forehead_x:forehead_x + coin_width]
            alpha_s = coin_resized[:, :, 3] / 255.0
            alpha_l = 1.0 - alpha_s
            for c in range(3):
                try:
                    coin_region[:, :, c] = (
                            alpha_s * coin_resized[:, :, c] + alpha_l * coin_region[:, :, c]
                    )
                except ValueError:
                    pass
