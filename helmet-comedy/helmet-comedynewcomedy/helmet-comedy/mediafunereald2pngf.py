...
while True:
    ...
    for i in range(len(boxes)):
        ...
        if results.multi_face_landmarks:
            ...
            for c in range(3):
                if y + height <= frame.shape[0] and x + width <= frame.shape[1]:
                    try:
                        face[y:y + height, x:x + width, c] = (
                                alpha_s * face_mesh_resized[:, :, c] + alpha_l * face[y:y + height, x:x + width, c]
                        )
                    except ValueError:
                        pass

        # Draw a white circle at the forehead
        cv2.circle(frame, (forehead_x, forehead_y), radius=5, color=(255, 255, 255), thickness=-1)

        # Draw a white circle at the nose nostrils
        nose_x = int(face_landmarks[5].x * frame.shape[1])
        nose_y = int(face_landmarks[5].y * frame.shape[0])
        ...
        hat_x = int(x + (width - hat_width) / 2)
        hat_y = int(y - hat_height * 0.5)
        ...
        alpha_l = 1.0 - alpha_s
        for c in range(3):
            if hat_y + hat_height <= frame.shape[0] and hat_x + hat_width <= frame.shape[1]:
                try:
                    hat_region[:, :, c] = (
